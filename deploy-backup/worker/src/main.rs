...
async fn compute(_req_body: web::Bytes) -> impl Responder {
...
async fn handle_generate_proof() -> impl Responder {
    match parse_static_json_data() {
        Ok(data_entries) => match generate_proof(&data_entries) {
            Ok(proof) => HttpResponse::Ok().body(proof),
            Err(e) => HttpResponse::InternalServerError().body(format!("Error generating proof: {:?}", e)),
        },
        Err(e) => HttpResponse::InternalServerError().body(format!("Error parsing data: {:?}", e)),
    }
}
...
async fn main() -> std::io::Result<()> {
    HttpServer::new(|| {
        App::new()
            ...
            .route("/generate_proof", web::post().to(handle_generate_proof)) // Adding new route
    })
    .bind("0.0.0.0:8081")?
    .run()
    .await
